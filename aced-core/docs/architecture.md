...
当前构建注册已集中在 aced-core/CMakeLists.txt，tests/CMakeLists.txt 已移除。2026-09-15 实测 MinGW x64 Debug 配置、构建及现有 5 个 CTest 全部通过；独立关闭 BUILD_TESTING 的目录也构建成功且注册测试数为 0。辅助函数的诊断文本已使用 ${name} 展开测试名称。tests/support/test_assertions.h 仍是现有测试设施，下面的框架重写方案尚未实施。

2026-09-15 用户确认的构建组织要求：所有本工程 CMake 构建规则放在 aced-core/CMakeLists.txt；内部按项目配置、源文件清单、辅助函数、生产目标、测试目标分组。CMakePresets.json 保留配置预设职责。目标声明采用 AsmJit 风格的 SOURCES、LIBRARIES、可选 INCLUDE_DIRS 关键字。辅助函数保留实际复用的构建操作，不扩展为没有实际需求的通用目标工厂。

本次可借鉴的细节：显式维护按职责分组的源码清单；用目标级 target_compile_features 表达 C++20 要求，避免重复声明全局标准版本；用 source_group(TREE ...) 整理支持该功能的 IDE 文件树，且不增加库目标；测试辅助函数检查未知参数、缺失值和空源文件清单，让错误在配置阶段暴露；测试支持和测试程序均受 BUILD_TESTING 控制。额外警告选项应按编译器配置并保持目标私有，复杂选项应检查支持情况；Sanitizer 需同时验证编译、链接与运行环境，另作专项任务，不照搬 AsmJit 针对代码生成器的指令集与优化选项。

教学约定：由用户逐步修改代码，每次安排一个可独立验证的小任务；完成后检查实时文件、架构边界和完整 CTest，再进入下一步。不要将本段计划当成已完成的迁移。

2026-09-15 测试重写方向（用户明确要求仅借鉴 AsmJit 与其他轻量框架的设计，不使用第三方测试框架，并确认继续由用户逐步编写；以下待实现及验证）：

- 借鉴 AsmJit 的用例注册与统一运行入口、doctest/Catch2 的具名用例、CHECK/REQUIRE 区分和失败上下文，自行编写 Aced 测试设施；不引入、复制或包装第三方框架。旧有“直接接入 doctest”的方案已被本次明确要求替代。
- 当前核心测试最终汇入一个运行程序，tests/test_main.cpp 只提供命令行入口，框架实现位于测试专用 support 模块。用例按套件与行为拆文件；生产核心不依赖框架，框架也不依赖 aced-core。后续 CLI、Java 互操作与模糊测试按运行环境和隔离需求另定目标。
- ByteReader 单独组织。serialization 内部按 MUTF-8 解码、流头、基础记录与通用限额、块数据、短/长字符串、引用与 reset 语义组织文件；同一行为的正常、边界、失败场景放在一起，不另建大而混杂的 error_test。
- 一个具名用例描述一个行为契约；独立用例不共享可变状态。同类表驱动输入必须带场景名，通过作用域上下文提供失败定位；上下文信息按需要保存快照，退出作用域恢复，不保留悬空引用。
- CHECK 记录失败后继续当前用例；REQUIRE 记录失败后通过专用控制异常结束当前用例，由运行器捕获后继续下一用例。解引用、数组访问等前提使用 REQUIRE；不能机械地将旧 require 全部改成 CHECK。不承诺从访问违规等进程崩溃中恢复。
- 模块样本工具只构造输入，不调用被测解析器生成预期，不暗藏断言。实际重复的纯字节工具才可移入 tests/support；无需保留没有职责的支持目录或目标。
- 设施职责：test_case.h 只描述套件名、用例名、函数入口和定义位置；TestContext 只持有一次用例执行的检查计数、失败记录和上下文；test_assertions.h 负责判断与必要值格式化，结果交给 TestContext；test_runner 负责注册、筛选、执行、汇总和终端输出，格式化报告先用内部函数，不创建报告插件体系。按这几个职责逐步形成头文件与实现，不为每个类单独建库。
- 用例函数接收 TestContext&，断言使用本次上下文，不通过全局“当前用例”指针传递可变状态。轻量 ACED_TEST_CASE/ACED_CHECK/ACED_REQUIRE 宏仅负责用例登记、表达式文本、位置与参数转发，实际逻辑放在函数中。比较使用显式 ACED_CHECK_EQ 等形式，保证操作数各求值一次，暂不做表达式模板分解。注册列表使用函数局部静态存储，执行前检查重名与空入口，按套件和名称稳定排序。
- 首版范围：具名用例、布尔/相等/预期异常断言、失败上下文、列出与筛选、终端摘要、可靠退出码。正常运行或筛选后没有用例匹配时返回失败；只列出用例时允许正常退出。暂不做嵌套 SECTION 的重放调度、多线程断言、插件报告器和测试继承体系。
- 验证设施自身的失败路径：CHECK 能继续、REQUIRE 能中止当前用例但后续用例仍运行、错误异常类型被报告、比较不重复求值、筛选无匹配不假通过、作用域上下文正确恢复。失败退出码用独立进程与预期输出核验，不能只靠设施自报通过。
- 重写测试表达方式，保留原有独立样本与预期，建立旧场景到新用例的对应记录；按模块完成迁移后再删除对应旧测试，不能靠删断言取得通过。框架用例数与 CTest 进程条目数分别报告。
...
- 当前用户已明确要求自行实现轻量测试设施，按上述范围渐进实现；保留根 CMake 集中构建与显式声明要求，先有可运行的最小设施再接入新测试目标，不注册空实现或一次删除现有测试。

运行测试前确认进程真的启动并执行断言。2026-09-06 曾出现 0xc0000135 运行环境错误及未完成的 stream_header 实现；这是历史记录，不代表当前状态。当前验证结果见本节开头，后续每步重新检查实际运行结果。
